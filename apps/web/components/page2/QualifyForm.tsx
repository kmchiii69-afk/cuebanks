...
function FreeFunnelScreen({ firstName }: { firstName: string }) {
  ...
  return (
    <div className="text-center pt-12 pb-8">
      <div className="font-mono text-[10px] font-bold text-acid tracking-[0.22em] uppercase mb-7">
        · Got it ·
      </div>
      <h2 className="font-display font-semibold text-[38px] leading-[1.05] tracking-[-0.03em] text-bone m-0 mb-5">
        {firstName ? `${firstName}, I want to make sure you get value.` : "I want to make sure you get value."}
      </h2>
      <p className="font-body text-[16px] leading-[1.65] text-ash max-w-[500px] mx-auto mb-9">
      ...
      <div className="flex items-center justify-center gap-2.5 text-ash font-mono text-[10px] font-bold tracking-[0.22em] uppercase">
  ...
  return (
    <div className="flex flex-col gap-7">
      ...
      <div>
        ...
        <div className="w-full h-[3px] bg-line overflow-hidden">
          <div
            style={{
              width: `${progress}%`,
              boxShadow: "0 0 18px rgba(249,255,60,0.5)",
            }}
      ...
      >
        ...
        {q.helper && (
          <div className="font-mono text-[10px] font-bold text-ash tracking-[0.22em] uppercase mb-3">
            · {q.helper}
          </div>
        )}

        <h2 className="font-display font-semibold text-[34px] leading-[1.1] tracking-[-0.025em] text-bone m-0 mb-7">
          {q.label}
        </h2>
      ...
      {/* Error */}
      {error && (
        <div className="px-4 py-3 bg-[rgba(255,45,171,0.10)] border border-pink text-bone font-body text-[14px]">
          {error}
        </div>
      )}

      {/* Nav */}
      <div className="flex justify-between items-center mt-3 pt-7 border-t border-line">
        <button
          onClick={goBack}
          ...
          className="px-4.5 py-3 bg-transparent border border-line-2 font-mono text-[11px] font-bold tracking-[0.22em] uppercase cursor-pointer"
        >
          ← Back
        </button>
        ...
        <button
          ...
          className={`px-5 py-3.5 border-0 font-mono font-bold tracking-[0.18em] uppercase transition-all duration-200 ${
            filled && !submitting ? "cursor-pointer" : "cursor-default"
          }`}
        >
      ...
      <div className="flex justify-center gap-4 font-mono text-[9px] font-bold text-muted tracking-[0.22em] uppercase">
        <span>· Enter to advance ·</span>
        <span>· ← back ·</span>
      </div>

      {isFinal && (
        <p className="mt-1 font-body text-[11px] leading-[1.55] text-muted text-center max-w-[560px] mx-auto">
        ...
        return (
          <button
            ...
            style={{
              ...
              borderColor: selected ? "var(--acid)" : "var(--line-2)",
              boxShadow: selected
                ? "0 0 0 1px var(--acid), 0 0 32px rgba(249,255,60,0.15)"
                : "none",
            }}
            className="flex items-center gap-4 px-5 py-4 text-bone text-left font-body text-[16px] leading-[1.4] cursor-pointer transition-all duration-150 border"
          >
            <span
              style={{
              ...
              className="w-7 h-7 flex-shrink-0 flex items-center justify-center font-mono text-[11px] font-bold tracking-[0.04em] border"
            >
              {selected ? (
  ...
  if (question.longform) {
    return (
      <textarea
        ...
        autoFocus
        maxLength={2000}
        className="w-full bg-bg-1 border border-line-2 text-bone py-4 px-5 font-body text-[16px] leading-[1.55] outline-none resize-y rounded-none box-border"
      />
    );
  }
  return (
    <input
      ...
      className="w-full bg-bg-1 border border-line-2 text-bone py-4 px-5 font-body text-[16px] outline-none rounded-none box-border"
    />
  );
}
